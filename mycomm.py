...
class CommNetMLP(nn.Module):
    ...
    def __init__(self, args, num_inputs):
        ...
        # Mask for communication - if comm_mask_zero = false in args add
        # add 1s
    ...
    def forward(self, x, info={}):
        ...
        batch_size = x.size()[0] # issue with self masking?
        n = self.nagents
        ...
        if self.args.hard_attn:
            ...
            comm_action_mask = comm_action.repeat(batch_size, n, n).unsqueeze(-1)
            # action 1 is talk, 0 is silent i.e. act as dead for comm purposes.
            # agent_mask = agent_mask.clone() * comm_action_mask.double()
            # agent_mask *= comm_action_mask.double()
            agent_mask = agent_mask * comm_action_mask.squeeze(-1).double()
